...
extension AppDelegate: ConnectionControllerDelegate {
    ...
    func didReceiveClipboard(text: String) {
        if !clipboardWriter.writeText(text) {
            appLogger.error("[App] Failed to write received text to the pasteboard")
        }
    ...
    func didEncounterError(error: ConnectionError) {
        ...
        case .peerRemovedPairing:
            showBluetoothAlert(
                ...
                info: "macOS may be holding an outdated system Bluetooth pairing for this phone, which can block BinderClip from connecting. Open Bluetooth settings, click the ⓘ next to the phone, choose \"Forget This Device,\" then pair again in BinderClip."
            )
        default: break
        }
    }
    ...
    func didUpdateBluetoothState(state: CBManagerState) {
        ...
        case .unauthorized:
            ...
            statusBarController.setBluetoothWarning("Allow Bluetooth access for BinderClip") { [weak self] in
    ...
    private func showBluetoothOffWarning() {
            statusBarController.setBluetoothWarning("Bluetooth is turned off - please enable Bluetooth for BinderClip to resume sync.") { [weak self] in
