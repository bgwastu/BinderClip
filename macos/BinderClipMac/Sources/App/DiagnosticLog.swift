...
// Logs to BinderClip can be read back regardless of how os_log is configured.
...
final class DiagnosticLog {
    ...
    private let queue = DispatchQueue(label: "net.wastu.clipboard.diagnostics")
    /// Mirror to os.Logger too, so Console still works during local development.
    private let logger = Logger(subsystem: "net.wastu.clipboard", category: "Diagnostics")
    ...
    private init() {
        ...
        let directory = (try? FileManager.default.url(
            ...
            appropriateFor: nil,
            create: true
        ))?.appendingPathComponent("BinderClip", isDirectory: true)
