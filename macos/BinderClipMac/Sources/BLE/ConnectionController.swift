...
class ConnectionController: NSObject {
    ...
    fileprivate let queue = DispatchQueue(label: "net.wastu.clipboard.connection")
    private let logger = Logger(subsystem: "net.wastu.clipboard", category: "Connection")
    ...
    /// Log every Clipboard advertisement we actually receive (throttled per tag),
    ...
    /// We saw a packet from a Clipboard device we can't act on, plus why. The
    ...
    /// targets) vs. a Clipboard payload that arrived but carried an unusable
    ...
    /// True if this manufacturer-data blob is a Clipboard advert: company ID
    ...
    static func isClipboardManufacturerData(_ data: Data) -> Bool {
...
extension ConnectionController: CBCentralManagerDelegate {
    ...
    func centralManager(
        ...
        // alone identifies a Clipboard device.
        guard let manufacturerData = advertisementData[CBAdvertisementDataManufacturerDataKey] as? Data,
              Self.isClipboardManufacturerData(manufacturerData),
            ...
            if let mfrData, Self.isClipboardManufacturerData(mfrData) {
                // Clipboard payload present but tag/PSM unusable (PSM=0): the
                // phone is advertising before its L2CAP server allocated a PSM.
                logUnusableAdvert(peripheral: peripheral, rssi: RSSI,
                                  reason: "Clipboard manufacturer data present but PSM=0 (L2CAP server not ready)")
