...
final class ConnectionControllerTests: XCTestCase {
    ...
    // MARK: - Clipboard Manufacturer Data Filter Tests

    func testIsClipboardManufacturerDataAcceptsValidPayload() {
        let data = Data([0xFF, 0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00, 0x83])
        XCTAssertTrue(ConnectionController.isClipboardManufacturerData(data))
    }

    func testIsClipboardManufacturerDataRejectsOtherCompanyID() {
        // Apple company ID (0x004C, little-endian 0x4C 0x00) with a full-length
        // payload must not be mistaken for a Clipboard tag+PSM under broad scan.
        let data = Data([0x4C, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00, 0x83])
        XCTAssertFalse(ConnectionController.isClipboardManufacturerData(data))
    }

    func testIsClipboardManufacturerDataRejectsShortData() {
        let data = Data([0xFF, 0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00])
        XCTAssertFalse(ConnectionController.isClipboardManufacturerData(data))
    }

    func testIsClipboardManufacturerDataRejectsEmptyData() {
        XCTAssertFalse(ConnectionController.isClipboardManufacturerData(Data()))
    }
