...
// ClipboardService to read and forward the clipboard.
//
// Detection strategy:
...
import net.wastu.clipboard.BuildConfig
...
class ClipboardAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (!ClipboardService.anyMacConnected) return
    ...
    private fun handleSystemUiWindow(event: AccessibilityEvent) {
        ...
        // here and a false positive costs the user a visible "Clipboard pasted
    ...
    private fun notifyService() {
        val intent = Intent(this, ClipboardService::class.java).apply {
            action = ClipboardService.ACTION_ACCESSIBILITY_COPY_DETECTED
        }
        ...
        runCatching { startService(intent) }
            .onFailure { Log.w(TAG, "Could not notify ClipboardService", it) }
    }
