...
class DirectClient(
    ...
    @Volatile private var inboundImage: IncomingImage? = null
    ...
    fun pair(uri: String) {
        ...
        val inviteKey = Base64.getUrlDecoder().decode(parsed.getQueryParameter("key") ?: error("Pairing code has no key"))
    ...
    private fun pair(host: String, port: Int, id: String, inviteKey: ByteArray) {
        ...
        require(DirectProtocol.constantTimeEquals(accepted.getString("proof"), DirectProtocol.hmac(inviteKey, "server|$id|$clientNonce|$serverNonce"))) { "Mac authentication failed" }
        val welcome = DirectProtocol.open(DirectProtocol.read(input), DirectProtocol.pairSessionKey(inviteKey, clientNonce, serverNonce))
        ...
        val pairedMac = RememberedPeer(welcome.optString("name", "Mac"), host, port, welcome.optString("deviceID"), "macOS", true)
    ...
    fun sendImage(image: ImagePayload) {
        ...
        if (outboundImage != null) { fail("An image is already sending"); return }
        outboundImage = image; outboundNextIndex = 0; outboundAcknowledgedIndex = -1
        write(out, DirectProtocol.seal(JSONObject().put("type", "mediaOffer").put("id", image.id).put("mime", image.mimeType).put("bytes", image.data.size).put("sha256", image.sha256), key))
    ...
    private fun attach(newSocket: Socket, input: DataInputStream, newOutput: DataOutputStream, key: ByteArray) {
        ...
        readThread = Thread {
            try {
                while (!newSocket.isClosed) {
                    ...
                    when (message.optString("type")) {
                        ...
                        "rosterRemove" -> {
                            ...
                            if (id == store.deviceId) { store.reset(); close(); onRosterChanged(emptyList()); onStatus("You left the BinderClip chain") }
                            else { store.removeMember(id); onRosterChanged(store.members) }
                        }
                        ...
                        "mediaReject" -> { outboundTimeout?.cancel(false); outboundTimeout = null; outboundImage = null; fail("Image rejected by Mac") }
                        "mediaAbort" -> { outboundTimeout?.cancel(false); outboundTimeout = null; inboundImage = null; outboundImage = null; fail("Image transfer cancelled") }
                    }
                }
            } catch (error: Exception) {
                ...
                if (socket === newSocket) { connected.set(false); output = null; onDisconnected() }
            }
        }.apply { name = "BinderClip direct receiver"; isDaemon = true; start() }
    }
    private fun sendHello() {
        val key = store.groupKey ?: return; val out = output ?: return
        write(out, DirectProtocol.seal(JSONObject().put("type", "hello").put("deviceID", store.deviceId).put("name", deviceName), key))
    }
    ...
    private fun handleImageOffer(message: JSONObject, key: ByteArray) {
        ...
        if (inboundImage != null || id.isBlank() || mime !in ImagePayload.ALLOWED_MIME_TYPES || bytes !in 1..ImagePayload.MAXIMUM_BYTES || hash.length != 64) {
            write(out, DirectProtocol.seal(JSONObject().put("type", "mediaReject").put("reason", "Unsupported image"), key)); return
        }
    ...
    private fun handleImageChunk(message: JSONObject, key: ByteArray) {
        ...
        if (id != incoming.id || index != incoming.nextIndex || chunk == null || chunk.isEmpty() || chunk.size > ImagePayload.CHUNK_BYTES || incoming.data.size() + chunk.size > incoming.bytes) {
            inboundImage = null; write(out, DirectProtocol.seal(JSONObject().put("type", "mediaAbort"), key)); fail("Image transfer failed"); return
        }
    ...
    private fun handleImageComplete(message: JSONObject) {
        ...
        inboundImage = null
        if (image == null || image.sha256 != incoming.sha256) { fail("Image verification failed"); return }
    ...
    private fun sendImageWindow(image: ImagePayload, key: ByteArray) {
        ...
        while (outboundNextIndex < totalChunks && outboundNextIndex <= outboundAcknowledgedIndex + MAX_IN_FLIGHT_IMAGE_CHUNKS) {
            ...
            write(out, DirectProtocol.seal(JSONObject().put("type", "mediaChunk").put("id", image.id).put("index", outboundNextIndex).put("data", Base64.getEncoder().encodeToString(image.data.copyOfRange(offset, end))), key))
            outboundNextIndex += 1
        }
        if (outboundAcknowledgedIndex + 1 >= totalChunks) {
            ...
            outboundTimeout?.cancel(false); outboundTimeout = null; outboundImage = null; onTransferStatus("Image sent"); return
        }
    ...
    private fun scheduleOutboundTimeout(imageId: String, index: Int, key: ByteArray) {
        ...
        outboundTimeout = transferScheduler.schedule({
            ...
            output?.let { write(it, DirectProtocol.seal(JSONObject().put("type", "mediaAbort").put("id", imageId), key)) }
            outboundImage = null
